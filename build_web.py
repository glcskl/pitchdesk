...
.car .qcard{position:absolute;top:0;left:50%;width:min(360px,72vw);padding:34px 26px 30px;
  ...
  transition:transform .6s cubic-bezier(.22,.8,.36,1),opacity .45s ease,border-color .3s}
.car .qcard.c-c{transform:translate(-50%,0) scale(1);opacity:1;z-index:3;pointer-events:auto}
...
<section class="slide">
  <div class="inner" style="display:flex;flex-direction:column;justify-content:center">
    <h1 class="rv" style="--d:.06s">Задачи от <em>компаний</em></h1>
...
<section class="slide">
  <div class="inner" style="display:flex;flex-direction:column;justify-content:center">
    <h1 class="rv" style="--d:.2s">Путь от идеи<br>до <em>стартапа</em></h1>
    <div class="road">
      <div class="rd rv" style="--d:1.2s"><div class="dot"></div><div class="step">01</div><h4>Идея</h4><p>или просто интерес — этого достаточно</p></div>
      <div class="rd rv" style="--d:2.2s"><div class="dot"></div><div class="step">02</div><h4>Хакатон</h4><p>команда и первый опыт за 48 часов</p></div>
      <div class="rd rv" style="--d:3.2s"><div class="dot"></div><div class="step">03</div><h4>Space University</h4><p>акселерация и питчинг</p></div>
      <div class="rd rv" style="--d:4.2s"><div class="dot"></div><div class="step">04</div><h4>Generation</h4><p>форум и демо-день</p></div>
      <div class="rd rv" style="--d:5.2s"><div class="dot"></div><div class="step">05</div><h4>Резидент</h4><p>поддержка и рост проекта</p></div>
    </div>
    <p class="lead rv" style="--d:6.2s;margin-top:40px;max-width:600px">Хакатон — лучший «первый шаг»: он не требует ни идеи, ни готовой команды.</p>
  </div>
</section>
...
<section class="slide">
  <div class="inner" style="display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center">
    ...
    <div class="car rv" id="car" style="--d:.18s">
      <div class="qcard c-c">
...
(function(){
  ...
  if(car){
    var citems=[].slice.call(car.querySelectorAll('.qcard')),cidx=0,tt;
    function cs(){
      citems.forEach(function(el,i){
        var r=(i-cidx+citems.length)%citems.length;
        el.classList.toggle('c-c',r===0);
        el.classList.toggle('c-r',r===1);
        el.classList.toggle('c-l',r===2);
      });
    }
    function cgo(d){cidx=(cidx+d+citems.length)%citems.length;cs();crestart()}
    function crestart(){clearInterval(tt);tt=setInterval(function(){cgo(1)},8400)}
    car.addEventListener('mouseenter',function(){clearInterval(tt)});
    car.addEventListener('mouseleave',crestart);
    cs();
    crestart();
  }
  render();
